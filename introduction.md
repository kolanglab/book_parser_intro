...
> [!NOTE]
> 「正規表現では括弧の対応をチェックできない」という有名な事実があります。これは理論的な裏付けがあり、第6章「パーサの理論」で**文脈自由文法**と**正規言語**の違いとして詳しく説明します（正確には理論上の正規表現の話で、後方参照や再帰のような拡張を持つ実用の正規表現エンジンとの違いも第6章で触れます）。いまは「正規表現だけでは足りない場面がある」とだけ覚えておいてください。
...
つまり AST は、テキストという「人間に優しい表現」と、後続処理が扱いやすい「機械に優しい表現」をつなぐ**ハブ**なのです。現代のコンパイラ理論の標準的な教科書であるドラゴンブック[](#cite:aho2006)も、コンパイラの構成をこの AST を中心に据えて説明しています。AST という共通の中間表現があるからこそ、フロントエンド（パース）とバックエンド（実行・生成）を分離して設計できるのです。
...
ここで重要なのは、`23` という2文字が1個の「整数トークン」にまとめられ、意味のない空白が捨てられている点です。字句解析を担当するプログラムを **字句解析器（lexer, スキャナ）** と呼びます。字句解析器がやっているのは、いわば文章を「単語」に区切る作業です。トークンの種類（整数なのか識別子なのか）を **トークン種別**、その具体的な文字列（`23` や `x`）を **字句（lexeme）** と呼びます。
