...
> [!TIP]
> 演算子が増えて優先順位が複雑になると、`expr`／`term`／`factor`……と階層関数を積み上げる方式は段々つらくなります。これを優先順位を**数値**として扱うことでスマートに捌く **Pratt 構文解析（演算子順位解析）** という技法があり、Vaughan Pratt が1973年に提案しました[Pratt, 1973](#cite:pratt1973)。多くの実用パーサ（たとえばインタプリタ入門書の作例や JavaScript エンジン）が式の解析にこの手法を採用しています。再帰下降の親戚として、第7章でもう一度触れます。
